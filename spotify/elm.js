...
Elm.Events.make = function (_elm) {
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var onChange = F2(function (address,
   f) {
...
Elm.Search.make = function (_elm) {
   ...
   if (_elm.Search.values)
   return _elm.Search.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var resultView = function (answer) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("panel panel-info")]),
      _L.fromArray([A2($Html.div,
                   _L.fromArray([$Html$Attributes.$class("panel-heading")]),
                   _L.fromArray([$Html.text("Album")]))
                   ,A2($Html.div,
                   _L.fromArray([$Html$Attributes.$class("panel-body")
                                ,$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                                      ,_0: "height"
                                                                      ,_1: "10rem"}]))]),
                   _L.fromArray([$Html.text(answer.name)]))]));
   };
   var bootstrap = A3($Html.node,
   "link",
   _L.fromArray([$Html$Attributes.href("https://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/css/bootstrap.min.css")
                ,$Html$Attributes.rel("stylesheet")]),
   _L.fromArray([]));
   var row = $Html.div(_L.fromArray([$Html$Attributes.$class("row")]));
   var resultsList = F2(function (address,
   model) {
      return function () {
         var toEntry = function (answer) {
            return A2($Html.div,
            _L.fromArray([$Html$Attributes.$class("col-xs-2 col-md-3")]),
            _L.fromArray([resultView(answer)]));
         };
         return row(A2($List.map,
         toEntry,
         model.answers));
      }();
   });
   var containerFluid = $Html.div(_L.fromArray([$Html$Attributes.$class("container-fluid")]));
   var RegisterAnswers = function (a) {
      return {ctor: "RegisterAnswers"
             ,_0: a};
   };
   var Query = {ctor: "Query"};
   var QueryChange = function (a) {
      return {ctor: "QueryChange"
             ,_0: a};
   };
   var inputForm = F2(function (address,
   model) {
      return A2($Html.input,
      _L.fromArray([$Html$Attributes.type$("text")
                   ,$Html$Attributes.placeholder("Search for an album...")
   ...
   model) {
      ...
      _L.fromArray([$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                         ,_0: "margin"
                                                         ,_1: "20px 0"}]))]),
      _L.fromArray([bootstrap
                   ,containerFluid(_L.fromArray([A2(inputForm,
                                                address,
                                                model)
                                                ,A2(resultsList,
                                                address,
   ...
   var decodeAnswers = function () {
      var albumDecoder = A3($Json$Decode.object2,
      Answer,
      A2($Json$Decode._op[":="],
      "name",
      $Json$Decode.string),
      ...
      _L.fromArray(["albums"
                   ,"items"]),
      $Json$Decode.list(albumDecoder));
   }();
   var search = function (query) {
      return $Effects.task($Task.map(RegisterAnswers)($Task.toMaybe(A2($Http.get,
      decodeAnswers,
      searchUrl(query)))));
   };
      ...
      return function () {
         ...
         {case "Query":
            ...
            return {ctor: "_Tuple2"
                   ...
                   ,_1: $Effects.none};}
         _U.badCase($moduleName,
         "between lines 49 and 63");
      }();
   });
   _elm.Search.values = {_op: _op
                        ,Model: Model
                        ,Answer: Answer
                        ,init: init
